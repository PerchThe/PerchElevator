...
public class Elevator {
   ...
   private Location getPotentialDestination(Location targetTop, Location target_bottom, Location blockFeet, Location blockBody) {
      if (this.combinationData.getBotMaterial() == null || SEMaterial.match(target_bottom.getBlock().getRelative(BlockFace.DOWN), false) == this.combinationData.getBotMaterial() && SEMaterial.match(target_bottom.getBlock().getRelative(BlockFace.DOWN), false).getData() == this.combinationData.getBotMaterial().getData()) {
         if (this.type != ElevatorType.PLATE && !SEMaterial.match(targetTop.getBlock().getRelative(BlockFace.DOWN), false).isSpecialType(SEMaterial.SpecialType.TRAPDOOR) && this.combinationData.getTopMaterial() != null && SEMaterial.match(targetTop.getBlock().getRelative(BlockFace.DOWN), false).getData() != this.combinationData.getTopMaterial().getData()) {
            return null;
         } else {
            if (this.type.equals(ElevatorType.BLOCK)) {
               if (Variables.getInstance().getIgnoreList() != null && (!Variables.getInstance().getIgnoreList().contains(SEMaterial.match(blockFeet.getBlock().getRelative(BlockFace.DOWN), true)) || !Variables.getInstance().getIgnoreList().contains(SEMaterial.match(blockBody.getBlock().getRelative(BlockFace.DOWN), true)))) {
                  return null;
               }
            } else if (this.type.equals(ElevatorType.NON_OCCLUDING) && Variables.getInstance().getIgnoreList() != null && !Variables.getInstance().getIgnoreList().contains(SEMaterial.match(blockFeet.getBlock().getRelative(BlockFace.DOWN), true))) {
               return null;
            }
            ...
            return new Location(this.player.getWorld(), (double)targetTop.getBlockX() + 0.5D, (double)targetTop.getBlockY() - offset, (double)targetTop.getBlockZ() + 0.5D, this.player.getLocation().getYaw(), this.player.getLocation().getPitch());
         }
      } else {
         return null;
      }
   ...
   private void elevate(Location destination, int elevatorDist) {
      ...
      if (!allowableDistance && !SEPerm.DISTANCE_BYPASS.hasPermission(this.player) && !this.playerMaxDist(this.player, elevatorDist)) {
         ...
         boolean vanished = this.player.getMetadata("vanished").stream().anyMatch(MetadataValue::asBoolean) || Bukkit.getServer().getPluginManager().isPluginEnabled("Essentials") && ((Essentials)Bukkit.getServer().getPluginManager().getPlugin("Essentials")).getUser(this.player).isVanished();
   ...
   private void tryLazyElevation() {
      ...
      if (lazyFloor != -1) {
         Location destination = new Location(this.player.getWorld(), (double)this.current.getBlockX() + 0.5D, (double)(lazyFloor + 1), (double)this.current.getBlockZ() + 0.5D, this.player.getLocation().getYaw(), this.player.getLocation().getPitch());
