...
public enum SEMaterial {
   ...
   RED_BED(14, new String[]{"BED_BLOCK", "BED"}),
...
// 1.21+
   ...
   private static final ImmutableMap<SEMaterial, SEMaterial> DUPLICATED = ImmutableMap.<SEMaterial, SEMaterial>builder().put(MELON, MELON_SLICE).put(CARROT, CARROTS).put(POTATO, POTATOES).put(BEETROOT, BEETROOTS).put(BROWN_MUSHROOM, BROWN_MUSHROOM_BLOCK).put(BRICK, BRICKS).put(RED_MUSHROOM, RED_MUSHROOM_BLOCK).put(MAP, FILLED_MAP).put(NETHER_BRICK, NETHER_BRICKS).put(DARK_OAK_DOOR, DARK_OAK_DOOR).put(ACACIA_DOOR, ACACIA_DOOR).put(BIRCH_DOOR, BIRCH_DOOR).put(JUNGLE_DOOR, JUNGLE_DOOR).put(SPRUCE_DOOR, SPRUCE_DOOR).put(CAULDRON, CAULDRON).build();
   ...
   public static SEMaterial match(Block block, boolean ignoreData) {
      if (block == null) {
         ...
         return !Variables.PRE_1_13 && block.getBlockData() instanceof DaylightDetector ? match(block.getType().name() + (((DaylightDetector)block.getBlockData()).isInverted() ? "_INVERTED" : "")) : match(block.getType().name() + (Variables.PRE_1_13 && !ignoreData ? ":" + block.getData() : ""));
      }
   }
   ...
   public static SEMaterial match(String material) {
      ...
      byte data = -1;
      material = split[0];
      if (split.length != 1 && Stream.of("DAYLIGHT_DETECTOR").noneMatch((s) -> {
         return split[0].toLowerCase().contains(s.toLowerCase());
      })) {
   ...
   private static SEMaterial matchDefinedSEMaterial(String name, byte data) {
      ...
      if (data <= 0 && (ISFLAT || !duplicated)) {
         ...
         if (xMat.isPresent()) {
            return (SEMaterial)xMat.get();
         }
      }

      return !ISFLAT && duplicated ? requestDuplicatedSEMaterial(name, data) : requestOldSEMaterial(name, data);
   }

   private static SEMaterial requestOldSEMaterial(String name, byte data) {
      String holder = name + data;
      SEMaterial material = (SEMaterial)NAME_CACHE.getIfPresent(holder);
      if (material != null) {
         return material;
      } else {
         ...
         do {
            do {
               ...
               materials = (SEMaterial)var4.next();
   ...
   private static SEMaterial requestDuplicatedSEMaterial(String name, byte data) {
      SEMaterial mat = requestOldSEMaterial(name, data);
      return mat == null ? null : (mat.name().charAt(mat.name().length() - 1) == 'S' ? (SEMaterial)Enums.getIfPresent(SEMaterial.class, name).orNull() : mat);
   }

   private static boolean isDuplicated(String name) {
      name = format(name);
      UnmodifiableIterator var1 = DUPLICATED.entrySet().iterator();
      ...
      do {
         ...
         duplicated = (Entry)var1.next();
      } while(!((SEMaterial)duplicated.getKey()).name().equals(name) && !((SEMaterial)duplicated.getKey()).anyMatchLegacy(name));
   ...
   public boolean isSpecialType(SEMaterial.SpecialType... specialTypes) {
      return Stream.of(specialTypes).anyMatch((specialType) -> {
         return this.name().contains(specialType.getContainedString());
      });
   }
